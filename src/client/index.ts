...
export class GeospatialIndex<K extends string = string> {
   ...
   */
  constructor(private component: UseApi<typeof api>, private maxResolution: number = DEFAULT_MAX_RESOLUTION) { }
   ...
   */
  async insert(ctx: GenericMutationCtx<GenericDataModel>, key: K, coordinates: Point) {
    await ctx.runMutation(this.component.index.insert, { key, coordinates, maxResolution: this.maxResolution })
  }
   ...
   */
  async get(ctx: GenericQueryCtx<GenericDataModel>, key: K): Promise<Point | null> {
   ...
   */
  async remove(ctx: GenericMutationCtx<GenericDataModel>, key: K): Promise<boolean> {
    return await ctx.runMutation(this.component.index.remove, { key, maxResolution: this.maxResolution })
  }
   ...
   * 
   * @param ctx - The Convex query context.
   * @param rectangle - The geographic area to query, specified as the four corners of the rectangle
   * in clockwise order.
  ...
  async queryRectangle(
    ...
    maxRows: number = 64,
  ): Promise<{ results: Array<{ key: K, coordinates: Point }>, h3Cells: string[] }> {
    const resp = await ctx.runQuery(
      this.component.index.queryRectangle, 
      { rectangle, maxRows, maxResolution: this.maxResolution },
    );    
    return resp as any;
  }
}
