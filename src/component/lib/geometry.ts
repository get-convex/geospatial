import { cellToParent, getHexagonEdgeLengthAvg, greatCircleDistance, gridDisk, latLngToCell, polygonToCells, UNITS } from "h3-js";
...
export function validateRectangle(rectangle: Rectangle) {
  ...
  if (sw.latitude !== se.latitude || nw.latitude !== ne.latitude) {
    throw new Error("Invalid rectangle: latitudes for top and bottom edges must match");    
  }
...
export function coverRectangle(rectangle: Rectangle, maxResolution: number): Set<string> | null {
