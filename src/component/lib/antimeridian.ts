// Port of https://github.com/gadomski/antimeridian to JS.
...
function positionsEqual(p: Position, q: Position) {
    return p[0] == q[0] && p[1] == q[1];
}

// Helper class that fixes up a ring of points into valid GeoJSON:
// - Splits the ring across the antimeridian.
// - Handles rings that contain the north or south pole.
export class GeoJSONPolygon {
    ...
    constructor(coords: Position[]) {
        const normalized = normalizeLongitudes(coords);
        const segmented = segmentAcrossAntimeridian(normalized);

        let polygons;
        if (segmented.length) {
            const extended = extendOverPoles(segmented);
            polygons = buildPolygons(extended);        
        } else {
            polygons = [turf.polygon([normalized])]
        }
        ...
        for (const polygon of polygons) {
            ...
            if (turf.booleanClockwise(exteriorRing)) {
                throw new Error(
                    `Exterior ring is clockwise: ${JSON.stringify(exteriorRing)}`,
                );
            }
        }
        for (let i = 0; i < polygons.length; i++) {
            for (let j = i + 1; j < polygons.length; j++) {
                ...
                if (intersects) {
                    throw new Error(
                        `Polygons are not disjoint: ${JSON.stringify(a)} intersects with ${JSON.stringify(b)}`,
                    );
                }
            }
        }
        this.polygons = polygons;
        this.area = this.polygons.reduce(
            (acc, polygon) => acc + turf.area(polygon),
            0,
        );
    }

    overlaps(other: GeoJSONPolygon) {
        ...
        for (const polygon of this.polygons) {
            for (const otherPolygon of other.polygons) {
                if (
                    turf.booleanContains(polygon, otherPolygon) ||
                    turf.booleanContains(otherPolygon, polygon) ||
                    turf.booleanIntersects(polygon, otherPolygon)
                ) {
                    return true;
                }
            }
    ...
    contains(other: GeoJSONPolygon) {
        return other.polygons.every((polygon) =>
            this.polygons.some((p) => turf.booleanContains(p, polygon)),
        );
    }
    ...
    containsPoint(point: Point) {
        ...
        return this.polygons.some((polygon) =>
            turf.booleanPointInPolygon(p, polygon),
        );
    }    
}

/**
 * Normalize the longitudes in a ring to [-180, 180]. 
 */
    ...
    for (let i = 0; i < coords.length; i++) {
        ...
        if (isClose(currentLng, 180)) {
            if (Math.abs(currentLat) !== 90 && isClose(prevLng, -180)) {
                // Attempt to use a consistent representation of the antimeridian.
...
/**
 * Segment a ring into segments that cross the antimeridian. 
 */
function segmentAcrossAntimeridian(coords: Position[]): Position[][] {
    ...
    if (positionsEqual(coords.at(-1)!, segments[0][0])) {
...
/**
 * If a set of segments crosses over either of the poles, add the map corners to
 * the appropriate segments. 
 */
function extendOverPoles(
    segments: Position[][],
    forceNorthPole: boolean = false,
    forceSouthPole: boolean = false,
): Position[][] {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (
            startLng === -180 &&
            (leftStart === null || startLat < leftStart.latitude)
        ) {
            leftStart = { index: i, latitude: startLat };
        } else if (
            startLng === 180 &&
            (rightStart === null || startLat > rightStart.latitude)
        ) {
        ...
        if (endLng === -180 && (leftEnd === null || endLat < leftEnd.latitude)) {
            ...
            endLng === 180 &&
            (rightEnd === null || endLat > rightEnd.latitude)
        ) {
    ...
    const result: Position[][] = segments.map((segment) =>
        segment.map((p) => [...p]),
    );
    ...
    if (leftEnd) {
        if (
            ...
            !forceSouthPole &&
            !rightEnd &&
            (!leftStart || leftEnd.latitude > leftStart.latitude)
        ) {
            isOverNorthPole = true;
    ...
    if (rightEnd) {
        if (
            forceSouthPole &&
            !forceNorthPole &&
            (!rightStart || rightEnd.latitude < rightStart.latitude)
        ) {
            isOverSouthPole = true;
...
function buildPolygons(
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (startLng === endLng) {
            const isCandidate =
                (isRight &&
                    startLat > endLat &&
                    (!isSelfClosing(s) || endLat < startLat)) ||
                (!isRight &&
                    startLat < endLat &&
                    (!isSelfClosing(s) || endLat > startLat));
            if (isCandidate) {
    ...
    if (index) {
        ...
        if (!segment.every((p) => positionsEqual(p, segment[0]))) {
...
/**
 * Compute the crossing latitude for the line segment between two points that 
...
function crossingLatitude(start: Position, end: Position) {
    ...
    if (endLng > 0) {
        return round(
            startLat + ((180 - startLng) * latitudeDelta) / (endLng + 360 - startLng),
        );
    } else {
        return round(
            startLat + ((startLng + 180) * latitudeDelta) / (startLng + 360 - endLng),
        );
    }
}
...
/**
 * Port of Python's `round(*, 7)` to JS.
 */
